@extends('template')

@section('title', __('messages.menus.purchases'))

@push('css')
    <link href="https://cdn.jsdelivr.net/npm/simple-datatables@latest/dist/style.css" rel="stylesheet" type="text/css">
@endpush

@section('content')
<div class="container-fluid px-4">
    <h1 class="mt-4">{{ __('messages.menus.purchases') }}</h1>
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item"><a href="{{ route('panel') }}">{{ __('messages.menus.core') }}</a></li>
        <li class="breadcrumb-item active">{{ __('messages.menus.purchases') }}</li>
    </ol>

    <div class="mb-4">
        <a href="{{ route('compras.create') }}">
            <button type="button" class="btn btn-primary">{{ __('messages.buttons.add_new_record') }}</button>
        </a>
    </div>

    <div class="card mb-4">
        <div class="card-header">
            <i class="fas fa-table me-1"></i>
            {{ __('messages.purchases.table') }}
        </div>
        <div class="card-body">
            <table id="datatablesSimple" class="table table-striped">
                <thead>
                    <tr>
                        <th>{{ __('messages.forms.fields.voucher') }}</th>
                        <th>{{ __('messages.forms.fields.supplier') }}</th>
                        <th>{{ __('messages.forms.fields.date_hour') }}</th>
                        <th>{{ __('messages.columns.total') }}</th>
                        <th>{{ __('messages.columns.actions') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($compras as $item)
                        <tr>
                            <td>
                                <p class="fw-semibold mb-1">{{ $item->comprobante->tipo_comprobante }}</p>
                                <p class="text-muted mb-0">{{ $item->numero_comprobante }}</p>
                            </td>
                            <td>
                                <p class="fw-semibold mb-1">{{ ucfirst($item->proveedore->persona->tipo_persona) }}</p>
                                <p class="text-muted mb-0">{{ $item->proveedore->persona->razon_social }}</p>
                            </td>
                            <td>
                                {{
                                    \Carbon\Carbon::parse($item->fecha_hora)->format('d-m-Y') . ' ' .
                                    \Carbon\Carbon::parse($item->fecha_hora)->format('H:i')
                                }}
                            </td>
                            <td>
                                {{ $item->total }}
                            </td>
                            {{-- <td>
                                <x-columna-estado :estado="$item->caracteristica->estado" />
                            </td> --}}
                            <td class="text-center">
                                <x-columna-acciones
                                    :ruta="'compras.edit'"
                                    :estado="$item->estado"
                                    :key="'compra'"
                                    :value="$item"
                                    :botonVer="'1'"
                                    :botonEdit="'0'"
                                />
                            </td>
                        </tr>

                        <!-- Confirm Modal -->
                        <x-confirm-modal
                            :ruta="'compras.destroy'"
                            :estado="$item->estado"
                            :key="'compra'"
                            :id="$item->id"
                            :value="$item->id"
                            :type="'purchase'"
                        />
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
</div>
@endsection


@push('js')
    <script src="https://cdn.jsdelivr.net/npm/simple-datatables@latest" type="text/javascript"></script>
    <script src="{{ asset('js/datatables-simple-demo.js') }}"></script>
@endpush