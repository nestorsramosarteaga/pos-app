...
<div class="container-fluid px-4">
    <h1 class="mt-4">{{ __('messages.suppliers.edit') }}</h1>
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item"><a href="{{ route('panel') }}">{{ __('messages.menus.core') }}</a></li>
        <li class="breadcrumb-item"><a href="{{ route('proveedores.index') }}">{{ __('messages.menus.customers') }}</a></li>
        <li class="breadcrumb-item active">{{ __('messages.suppliers.edit') }}</li>
    </ol>
        ...
        <form action="{{ route('proveedores.update',['proveedore'=>$proveedore]) }}" method="post">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <span class="fw-bold">{{ strtoupper($proveedore->persona->tipo_persona) }}</span>
                </div>
                    ...
                    >
                        @foreach ($documentos as $item)
                            <option value="{{$item->id}}" {{
                                (old('documento_id') == $item->id || $proveedore->persona->documento_id == $item->id )
                                    ? 'selected'
                                    : '' }}
