...
<div class="container-fluid px-4">
    <h1 class="mt-4">{{ __('messages.products.edit') }}</h1>
    <ol class="breadcrumb mb-4">
        ...
        <li class="breadcrumb-item active">{{ __('messages.products.edit') }}</li>
    </ol>
        ...
        <form action="{{route('productos.update',['producto'=>$producto])}}" method="post" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-12 mb-2">
                    ...
                    <textarea name="descripcion" id="descripcion" rows="3" class="form-control textarea-fixed">{{old('descripcion', $producto->descripcion)}}</textarea>
                    @error('descripcion')
                        <small class="text-danger">{{'*' . $message}}</small>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-2">
                    ...
                    >
                        @foreach ($marcas as $item)
                            <option value="{{$item->id}}" {{ (old('marca_id') == $item->id || $producto->marca_id == $item->id) ? 'selected' : '' }}>{{$item->nombre}}</option>
                        @endforeach
                    </select>
                    @error('marca_id')
                        <small class="text-danger">{{'*' . $message}}</small>
                ...
                <div class="col-md-6 mb-2">
                    ...
                    >
                        @foreach ($presentaciones as $item)
                            <option value="{{$item->id}}" {{ (old('presentacione_id') == $item->id || $producto->presentacione_id == $item->id) ? 'selected' : '' }}>{{$item->nombre}}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-2">
                    ...
                    >
                        @foreach ($categorias as $item)
                            <option value="{{$item->id}}" {{ (in_array($item->id, old('categorias',[])) || in_array($item->id,$producto->categorias->pluck('id')->toArray())) ? 'selected' : ''}}>{{$item->nombre}}</option>
                        @endforeach
                    </select>
                    @error('categorias')
                        <small class="text-danger">{{'*' . $message}}</small>
